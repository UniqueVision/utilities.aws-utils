...
use aws_sdk_athena::{
    Client, operation::get_query_execution::GetQueryExecutionOutput, types::QueryExecutionState,
};

use crate::{
    error::Error,
    query::{get_query_execution, start_query_execution},
};

pub async fn check_query_succeeded(
    client: &Client,
...
pub async fn execute_query_for_stream(
    ...
    let query_execution_id = start_query_execution(client, Some(query_string), None, None).await?;
    let query_execution_id = query_execution_id
        .query_execution_id()
        .ok_or_else(|| Error::Invalid("query execution ID is missing".to_owned()))?
        .to_string();

    // Athena側でのクエリ完了までポーリングする
    tokio::time::timeout(
        timeout_duration,
